...
def um_contra_todos(experimento,contador,treino_teste,parametros_svm):
	...
	for i, classe in enumerate(teste[-1]):
		...
		#-----------------------------------------------------
	# --------------------------------------------------------
	if not os.path.exists(str(experimento)+'/executions/outputs_svm/'):
		os.makedirs(str(experimento)+'/executions/outputs_svm/')
	exportar_csv(matriz_saidas, experimento+'/executions/outputs_svm/execution_'+repr(contador)+'_svm.csv')
	# --------------------------------------------------------
